  ...
  try {
    ...
    if (!lite && (!role || role === 'all' || role === 'student')) {
      ...
      if (studentIds.length > 0) {
        ...
        const logsSummary = await db.attendanceLog.groupBy({
          by: ['studentId', 'status'],
          _count: {
            status: true
          },
        ...
        for (const log of logsSummary) {
          if (!logsMap.has(log.studentId)) {
            logsMap.set(log.studentId, { attended: 0, total: 0 });
          }
          const stats = logsMap.get(log.studentId);
          const count = log._count.status;
          stats.total += count;
        ...
        for (const att of semesterAttendance) {
          if (!attMap.has(att.studentId)) {
            attMap.set(att.studentId, []);
          }
